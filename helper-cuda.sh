...
function systemCheck()
{
    ...
    do
        IDX=$(nvidia-smi topo -m | grep "GPU$idx" | awk 'NR==2 { print $12 }')
...
function setupAffinity()
{
    ...
    echo -e "Local Rank = "$OMPI_COMM_WORLD_LOCAL_RANK"\tGOMP_CPU_AFFINITY = "$GOMP_CPU_AFFINITY"\t NUMA Node = "${Gpu_numa_affinity_list[${my_gpu}]}"\tCUDA_VISIBLE_DEVICES = "$CUDA_VISIBLE_DEVICES
    # echo -e "Local Rank = "$OMPI_COMM_WORLD_LOCAL_RANK"\tOMP_PLACES = "$OMP_PLACES"\t NUMA Node = "${Gpu_numa_affinity_list[${my_gpu}]}"\tCUDA_VISIBLE_DEVICES = "$CUDA_VISIBLE_DEVICES
}
