...
void * provide_umpire_pool(size_t N)
{
  ...
  if (INITIALIZED==0){
    ...
    umpire_aligned_alloc = umpire_rm->getAllocator("DEVICE");
    umpire_pooled_allocator = umpire_rm->makeAllocator<umpire::strategy::DynamicPoolList>("DEVICE_pool", umpire_aligned_alloc,
