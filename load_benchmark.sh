...
function configure_lidCavity()
{
	...
	FVPATH=HPC_Benchmark/incompressible/icoFoam/cavity3D/8M/fixedIter/system/fvSolution
	sed -i "s/mat_type.*mpiaijhipsparse/mat_type mpiaijcusparse/g" $FVPATH
	sed -i "s/vec_type.*hip/vec_type cuda/g" $FVPATH 
    fi
...
function configure_hpcMotorbike()
{
    ...
    if [[ -v CUDA ]] && [[ $CUDA -eq 1 ]]; then 
	FVPATH=HPC_Benchmark/incompressible/simpleFoam/HPC_motorbike/Small/v1912/system/fvSolution
	sed -i "s/mat_type.*mpiaijhipsparse/mat_type mpiaijcusparse/g" $FVPATH
	sed -i "s/vec_type.*hip/vec_type cuda/g" $FVPATH
    fi
